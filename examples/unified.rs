use env_logger;
use kowalski::{Agent, Config, agent::UnifiedAgent};
use log::info;
use serde_json;
                ...
                if let Some(tool_calls) = message.tool_calls {
                    for tool_call in tool_calls {
                        ...
                        if function_name == "search" {
                            ...
                            info!("Query: {:?}", query);
                            let cache = function_arguments
                                .get("use_cache")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Cache: {:?}", cache);
                            let max_results = function_arguments
                                .get("max_results")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Max results: {:?}", max_results);
                            let include_images = function_arguments
                                .get("include_images")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Include images: {:?}", include_images);
                            let include_videos = function_arguments
                                .get("include_videos")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Include videos: {:?}", include_videos);
                            let include_news = function_arguments
                                .get("include_news")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Include news: {:?}", include_news);
                            let include_maps = function_arguments
                                .get("include_maps")
                                .unwrap_or(&serde_json::Value::Null);
                            info!("Include maps: {:?}", include_maps);
                            let include_shopping = function_arguments
                                .get("include_shopping")
                                .unwrap_or(&serde_json::Value::Null);
