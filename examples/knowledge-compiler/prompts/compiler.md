...
- Optional excerpts from an external vault (`external_vault_root` / mdBook `doc/`), when provided by the runner.
- Existing pages under `wiki/concepts/` and `wiki/summaries/`.

Tasks:
1. Extract key concepts, entities, and claims.
2. Reuse existing concept titles when they appear in the corpus excerpts or wiki — prefer matching `[[Exact Title]]` over inventing new names.
