...
When delegated `kc.lint`, the worker reads the wiki index, asks the LLM for an integrity report, writes the result under `debug/lint/`, then writes **`PASTE_ME.md`** at the workdir root.
