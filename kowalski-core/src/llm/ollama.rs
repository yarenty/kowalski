...
impl LLMProvider for OllamaProvider {
    async fn chat(&self, model: &str, messages: &[Message]) -> Result<String, KowalskiError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(KowalskiError::Server(format!("Ollama error: {}", error_text)));
        }
        ...
        let content = response_json["message"]["content"]
            .as_str()
            .ok_or(KowalskiError::Server("No content in Ollama response".to_string()))?
            .to_string();
    ...
    async fn embed(&self, text: &str) -> Result<Vec<f32>, KowalskiError> {
        ...
        let response = self.client.post(&url)
            ...
            .send()
            .await
            .map_err(|e| KowalskiError::Memory(format!("Failed to call Ollama embedding: {}", e)))?;
        ...
        let embedding = json["embedding"]
            .as_array()
            .ok_or(KowalskiError::Memory("No embedding field in response".to_string()))?
            .iter()
