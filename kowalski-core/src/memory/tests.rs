...
async fn test_memory_isolation() {
    ...
    let mut agent1 = BaseAgent::new(config1, "agent1", "Test Agent 1", wm1, em1.clone(), sm1).await.unwrap();
    ...
    let mut agent2 = BaseAgent::new(config2, "agent2", "Test Agent 2", wm2, em2.clone(), sm2).await.unwrap();
    ...
    agent1.working_memory.lock().await.add(memory1).await.unwrap();

    // Verify Agent 1 has it
    let results1 = agent1.working_memory.lock().await.retrieve("Secret", 10).await.unwrap();
    ...
    let results2 = agent2.working_memory.lock().await.retrieve("Secret", 10).await.unwrap();
    assert!(results2.is_empty(), "Agent 2 should NOT find Agent 1's memory");
    ...
    agent2.working_memory.lock().await.add(memory2).await.unwrap();

    // Verify Agent 2 has it
    let results2_b = agent2.working_memory.lock().await.retrieve("Secret", 10).await.unwrap();
    assert!(!results2_b.is_empty());
    assert_eq!(results2_b[0].content, "Secret 2 for Agent 2");
    
    // Verify Agent 1 still has only its own
    let results1_b = agent1.working_memory.lock().await.retrieve("Secret", 10).await.unwrap();
    ...
    println!("Episodic Memory Isolation: Setup successful (Validation skipped due to external dependency)");
}
