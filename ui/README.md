...
| 5b | **Rookery / Horde** (DAG, optional) | Open born [`examples/dag-demo/`](../examples/dag-demo/) in **Rookery** (reload session) or restart server and pick **dag-demo** in **Horde** | Canvas shows fork/join layers + read-only edge list; **Horde Run** shows DAG scheduling note. |
