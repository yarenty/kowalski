...
- **DAG scheduling (1.4.0):** `agent-app run` calls `kowalski_core::resolve_execution_graph` + `execution_order`; federation workers populate `@step:name@` via `collect_step_paths`. See [`examples/dag-demo/`](../examples/dag-demo/).
...
### Current Status
**1.3.0**: unchanged operator surface; **`mcp ping`/`mcp tools`** verified against **`kowalski-mcp-rookery`** and Docker MCP gateway. **`agent-app`** + federation helpers unchanged.
